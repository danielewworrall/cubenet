...
def optimize(n_calls):
    ...
    dimensions = [(16,64),                      # batch size
                  ...
                  (1,6),                        # n_radii
                  (0.5/np.pi,1.)]               # sigma
    x0 = [32,15,32,1e-3,5,2,4,4,1./np.pi]
...
def wrapper_function(dimensions):
    ...
    parser.add_argument("--save_dir", help="directory to save results", default="./models/VMar4SB_0/checkpoints")
    parser.add_argument("--log_dir", help="directory to save results", default="./models/VMar4SB_0/logs")
    ...
    args.max_degree = int(dimensions[6])
    args.n_radii = int(dimensions[7])
    args.sigma = float(dimensions[8])
