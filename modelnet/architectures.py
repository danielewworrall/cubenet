...
class GVGG(Layers):
    def __init__(self, args, images, is_training):
        super().__init__(args)
...
class GResnetDeep(Layers):
    ...
    def get_pred(self, x, is_training): #, reuse=False):
        with tf.variable_scope('prediction') as scope: #, reuse=reuse) as scope:
            ...
            x = self.Gres_block(x, self.ks, 16*nc, is_training, use_bn=use_bn, strides=2, drop_sigma=ds, name="Gres_4a")
            x = self.Gres_block(x, self.ks, 16*nc, is_training, use_bn=use_bn, drop_sigma=ds, name="Gres_4b")
            x = self.Gres_block(x, self.ks, 16*nc, is_training, use_bn=use_bn, drop_sigma=ds, name="Gres_4c")
            print(x)
